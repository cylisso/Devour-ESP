...
static int EnableHookLL(UINT pos, BOOL enable)
{
	PHOOK_ENTRY pHook = &g_hooks.pItems[pos];
	DWORD  oldProtect;
	...
	{
		if (pHook->patchAbove) memcpy(pPatchTarget, pHook->backup, sizeof(JMP_REL) + sizeof(JMP_REL_SHORT));
		else memcpy(pPatchTarget, pHook->backup, sizeof(JMP_REL));
	}
...
static int EnableAllHooksLL(BOOL enable)
{
	int status = 0;
...
static int EnableHook(LPVOID pTarget, BOOL enable)
{
	int status = 0;
